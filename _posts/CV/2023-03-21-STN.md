...
$$
\begin{pmatrix} x_i^s\\ y_i^s  \end{pmatrix} = T_{\theta}(G_i)=A_{\theta}\begin{pmatrix} x_i^t\\ y_i^t \\ 1  \end{pmatrix} = \begin{bmatrix} s & 0 & t_{x} \\ 0 & s & t_{y}   \end{bmatrix} \begin{pmatrix} x_i^t\\ y_i^t \\ 1  \end{pmatrix}
$$
...
$$
{{\partial V_i^c} \over {\partial x^s_i}}=\sum_n^H\sum_m^W U^c_{nm}max(0,1-\vert y^s_i-n \vert)\begin{cases}0 & if \  \vert m-x^s_i \vert \ge 1 \\1 & if \ m \ge x^s_i \\-1 & if \ m < x^s_i
\end{cases}
$$
