...
The goal of the generative model is to make the probability distribution \( P_g \) resemble the probability distribution of the actual data \( x \). For this purpose, suppose that the probability distribution \( P_z(z) \) of the noise variables \( z \) initially used by the generative model is defined.
